...
function serve(req, res) {
    ...
    console.log("fullExtName : " + fullExtName);
...
function findExtend(filePath, req, res) {
    ...
    function findFile(fullPath) {
        ...
        fs.stat(fullPath, function (err, stats) {
            ...
            if (stats && stats.isFile()) {
                // /pure.html
                render("/" + path.basename(fullPath), req, res);
            } else {
                i++;
                findFile(fullDotPath + extend[i]);
            }
        });
    }

    findFile(fullDotPath + extend[0]);
}
...
function render(fileName, req, res) {
    ...
    var pathName   = path.resolve(root + fileName); //  E:\Andrew\serve-komplete\public\index.html
    ...
    fileName = path.basename(fileName); //index.html fileName.slice(1, fileName.length);

    engine.render(fileName, data, function (err, file) {
        ...
        writeFile(fileName, file, function(err, file){
            /*res._headers = null;
            res.statusCode = 200;
